...
export function MemberDutyManager({ duties }: MemberDutyManagerProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border p-4">
        <h2 className="font-semibold">직분 목록</h2>
        <p className="mt-1 text-xs text-muted-foreground">
          `없음`은 명부 수정 화면의 기본 선택지라 여기서 삭제하지 않아요. 사용 중인 직분은 먼저 청년 명부에서 다른 직분으로 바꾼 뒤 삭제할 수 있어요.
        </p>
        <div className="mt-3 space-y-3">
          {duties.length === 0 ? (
            ...
            duties.map((duty) => (
              ...
              >
                <Input name="name" defaultValue={duty.name} maxLength={30} required aria-label="직분명" />
